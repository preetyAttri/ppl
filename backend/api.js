...
module.exports = {
  ...
  loginUserPassword: function(data) {
    return new Promise((res, rej) => {
      console.log("data in api loginUser", data);
      user_db.find({ email: data.email, psw: data.psw }, function(err, result) {
        if (err) {
          rej(err);
        } else {
          if (Object.keys(result).length === 0) {
  ...
  loginUserVerification: function(data) {
    return new Promise((res, rej) => {
      console.log("data in api loginUser", data);
      user_db.find({ email: data.email, verified: true }, function(
        err,
        result
      ) {
        if (err) {
  ...
  updatePassword: function(data) {
    return new Promise((res, rej) => {
      console.log("data in update password api ", data);
      user_db.updateOne(
        { email: data.email },
        { $set: { psw: data.psw } },
        function(err, result) {
